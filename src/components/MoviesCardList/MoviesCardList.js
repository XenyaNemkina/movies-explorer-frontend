    ...
    <section className="movieslist">
      <div className="movieslist__items">
        {displaySearchSavedMovies?.length && isSaved
          ? displaySearchSavedMovies?.map((el) => <MoviesCard data={el} id={el.movieId ? el.movieId : el._id} key={el.movieId + Math.random()} isSaved={true} deleteMovie={deleteMovie} />)
          : isSaved
          ? savedList?.map((el) => <MoviesCard data={el} id={el.movieId ? el.movieId : el._id} key={el.movieId + Math.random()} isSaved={true} deleteMovie={deleteMovie} />)
